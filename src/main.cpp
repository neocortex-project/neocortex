...
uint256 hashGenesisBlock("0xf0bab371df96a3cface3db70555e19ec49502806119a8ed2f001ae6bd286f035");
...
const string strMessageMagic = "Neocortex Signed Message:\n";
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Neocortex: 1 day
static const int64 nTargetSpacing = 90; // Neocortex: 1 and a half minute
...
// Thanks to Monocle
unsigned int static NeoDF(const CBlockIndex* pindexLast, uint32_t TargetBlocksSpacingSeconds, uint32_t PastBlocksMin, uint32_t PastBlocksMax) {
    ...
    // debug print
    printf("Difficulty Retarget - Pug Difficulty Function\n");
    printf("nHeight = %i\n", pindexLast->nHeight);
...
{
	static const uint32_t        BlocksTargetSpacing                        = 90; // 2 minutes
    ...
    return NeoDF(pindexLast, BlocksTargetSpacing, PastBlocksMin, PastBlocksMax);
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal neocortex miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Neocortex", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf0bab371df96a3cface3db70555e19ec49502806119a8ed2f001ae6bd286f035");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1406480001;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 388578842;
        ...
        assert(block.hashMerkleRoot == uint256("0x32a497aa2ace6d1c9bab1dbd484ed941e542f7d19076e1d85eaa7cfa54f426ed"));
        ...
        if (block.GetHash() != hashGenesisBlock)
        {
            ...
            loop
            {
                neoscrypt(UBEGIN(block.nVersion), UBEGIN(thash), 0x3);
                if (thash <= hashTarget)
                    break;
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    while (fGenerateBitcoins)
    {
        ...
        loop
        {
            ...
            loop
            {
                neoscrypt(UBEGIN(pblock->nVersion), UBEGIN(thash), 0x3);
